<?php 
    ...
    if(isset($_POST['add_room'])){
        ...
        if($stmt = mysqli_prepare($con, $query2)){
            ...
            $flag = 0;
            die('Query cannot be prepared - insert facility');
        }
    ...
    if(isset($_POST['get_all_rooms'])){
        ...
        while($row = mysqli_fetch_assoc($res)){
            if($row['status'] == 1){
                $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-dark btn-sm shadow-none'>Kích hoạt</button>";
            }
            else{
                $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-warning btn-sm shadow-none'>Ngưng kích hoạt</button>";
            }
            ...
            $data .= "
                <tr class='align-middle'>
                    ...
                    <td>$row[area]m2</td>
                    <td>
                        <span class='badge rounded-pill bg-light text-dark'>Adult: $row[adult] Người lớn</span><br>
                        <span class='badge rounded-pill bg-light text-dark'>Children: $row[children] Trẻ em</span>
                    </td>
                    <td>$row[price].000VNĐ</td>
                    <td>$row[quantity]</td>
                    <td>$status</td>
                    <td>
                        ...
                        <button type='button' onclick=\"room_images($row[id], '$row[name]')\" class='btn btn-info shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#room-images'>
                            <i class='bi bi-images'></i> Thêm ảnh
                        </button>
    ...
    if(isset($_POST['get_room'])){
        ...
        if(mysqli_num_rows($res2) > 0){
            ...
            $roomdata['facilities'] = $facilities;
        }
